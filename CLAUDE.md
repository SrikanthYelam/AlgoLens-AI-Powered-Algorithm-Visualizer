...
Backend (`/src`) and frontend (`/algolens-ui`) both implement 11 algorithms (see README.md): Binary Tree Level Order Traversal, Sliding Window Maximum, Largest Rectangle in Histogram, Number of Islands, Permutations, Combinations, Subsets, N-Queens, Letter Combinations of a Phone Number, Task Scheduler, Generate Parentheses.
...
* **`AlgoLens.Core`** (class library, no I/O, no AI dependency) — pure algorithm implementations. The central abstraction is the step-capture model in `Models/AlgorithmStep.cs`:
  ...
  - `IAlgorithmVisualizer<TInput>` (`Id` + `Run(TInput) -> IReadOnlyList<AlgorithmStep>`) is the contract every algorithm implements. All 11 follow the same shape — a dedicated `*State` record in `Models/` plus one class in `Algorithms/`:
...
* **`AlgoLens.Api`** (ASP.NET Core, minimal APIs — no controllers) — thin HTTP layer over Core:
  ...
  - Routes: `POST /api/algorithms/{binary-tree-level-order-traversal,sliding-window-maximum,largest-rectangle-in-histogram,number-of-islands,permutations,combinations,subsets,n-queens,letter-combinations-of-a-phone-number,task-scheduler,generate-parentheses}`, plus a `GET .../source` sibling of each (raw `.cs` text, for the frontend's `CodePanel`).
