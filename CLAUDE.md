...
* **`AlgoLens.Core`** (class library, no I/O, no AI dependency) — pure algorithm implementations. The central abstraction is the step-capture model in `Models/AlgorithmStep.cs`:
  ...
  - `IAlgorithmVisualizer<TInput>` (`Id` + `Run(TInput) -> IReadOnlyList<AlgorithmStep>`) is the contract every algorithm implements. All 19 follow the same shape — a dedicated `*State` record in `Models/` plus one class in `Algorithms/`:
